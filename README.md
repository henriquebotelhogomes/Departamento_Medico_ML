...
### Fase 2 — Fine-Tuning Seletivo (conv5_block)
...
- **Celery + Redis** — Worker assíncrono para inferência pesada, desacoplando do ciclo de request
- **Cache de predições** — SHA-256 hash → Redis com TTL, evitando re-inferência de imagens duplicadas
...
| **Backend** | Python 3.12 · FastAPI · Pydantic v2 · SQLAlchemy 2 (async) · Celery |
| **Frontend** | React 18 · Vite · TypeScript · Tailwind CSS · TanStack Query · i18next |
| **Database** | SQLite (dev) / PostgreSQL (prod) · Alembic migrations · Redis cache |
...
Consulte [PRD-V1.md](PRD-V1.md) para o plano detalhado de evolução. Itens futuros:
